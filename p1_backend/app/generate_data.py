...
Run once at startup (or offline) to produce data/history.csv and data/current_state.csv.
"""
import numpy as np
import pandas as pd
...
# Assign criticality + base demand/capacity profile per SKU (deterministic, reproducible)
rng = np.random.default_rng(42)
SKU_PROFILE = {}
for i, sku in enumerate(SKUS):
    SKU_PROFILE[sku] = {
        "criticality": "Critical" if i < 9 else "Non-Critical",  # first 9 critical
...
FLU_SEASON_MONTHS = {11, 12, 1, 2}  # Nov-Feb
    ...
    for sku in SKUS:
        ...
        for region in REGIONS:
            ...
            for d_idx in range(DAYS_HISTORY):
                ...
                flu_spike = rng.uniform(1.5, 1.6) if flu else 1.0  # +50-60% in flu season
                # Tier-2 flu spikes hit harder & stock is thinner -> stockouts there
                if is_tier2 and flu:
                ...
                # periodic replenishment (every ~lead_time days) so series doesn't collapse to 0
                ...
                sensed_signal = demand * rng.uniform(0.9, 1.1)  # proxy for IoT/POS sensing
